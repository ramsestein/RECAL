"""
adapt/tests/test_designer_rules.py
====================================
...
class TestShouldMaskFeatures:
    ...
    def test_activates_above_threshold(self):
        from adapt.designer.rules import should_mask_features
...
class TestQuantileTransformRule:
    ...
    def test_near_constant_excluded(self):
        from adapt.designer.rules import should_apply_quantile_transform_per_feature
        profile = _make_profile()
    ...
    def test_nonlinear_drift_with_high_cv_included(self):
        from adapt.designer.rules import should_apply_quantile_transform_per_feature
        from adapt.profiler.constants import CV_TARGET_QT_MINIMUM, VAR_RATIO_QT_UPPER
...
class TestWOERule:
    ...
    def test_deactivates_below_n_events_target(self):
        from adapt.designer.rules import should_apply_woe_per_feature
    ...
    def test_activates_stable_feature_above_threshold(self):
        from adapt.designer.rules import should_apply_woe_per_feature
        from adapt.profiler.constants import SHAP_WOE_MINIMUM
        features = [
            FeatureProfile(
...
class TestPCACoralRule:
    ...
    def test_always_activated(self):
        from adapt.designer.rules import should_apply_pca_coral
        profile = _make_profile()
        apply, _ = should_apply_pca_coral(profile)
        assert apply is True

    def test_k_respects_sqrt_cap(self):
        from adapt.designer.rules import select_pca_coral_k
        from adapt.profiler.constants import PCA_CORAL_K_RANGE_MIN
...
class TestCalibrationRule:
    ...
    def test_activates_on_bad_slope(self):
        from adapt.designer.rules import should_recalibrate
        profile = _make_profile(
    ...
    def test_platt_loo_default(self):
        from adapt.designer.rules import select_calibration_method
